...
<a href="http://rem-esp.es" target="_blank">
  <img src="Logo_REM-ESP_EA4RCR.png" alt="EA4RCR" style="height:70px;width:auto;">
</a>
...
<div class="display-row" style="margin-top:1.2rem;">
  ...
  <div id="nxdnDisplayPanel">
    ...
    <div class="nextion-nxdn">
      <div class="nx-topbar nxdn-bar"><span class="nx-mode">NXDN · DIGITAL</span><span style="color:#707000" id="nxdnStationLabel">EA4RCR · REM</span><span style="color:#ffd700;opacity:.85;min-width:5rem;text-align:right;font-size:.6rem;" id="nxdnTGLabel">—</span></div>
...
const UPDATE_TITLES={imagen:'🖼 Actualizar Imagen',ids:'📋 Actualizar IDS DMR',ysf:'📡 Actualizar Reflectores YSF'};
...
async function rebootPi(){if(!confirm('¿ Seguro que quieres reiniciar la Orangepi ?'))return;const btn=document.getElementById('btnReboot');btn.textContent='⏻ Reiniciando…';btn.disabled=true;try{await fetch('?action=reboot');}catch(e){}btn.textContent='⏳ Esperando arranque…';const start=Date.now();await new Promise(r=>setTimeout(r,8000));const poll=setInterval(async()=>{if(Date.now()-start>120000){clearInterval(poll);btn.textContent='⏻ Reiniciar Opi';btn.disabled=false;return;}try{const r=await fetch('?action=status',{cache:'no-store'});if(r.ok){clearInterval(poll);location.reload();}}catch(e){}},3000);}
